...
- [ ] BTC/KRW 캔들스틱 차트 (lightweight-charts, 90일 목데이터)
- [ ] 타임프레임 탭 (1분~1일)
- [ ] 봇 상태 카드 (3개 봇)
- [ ] 최근 체결 내역 미니 테이블
- [ ] 빌드 확인 + 린트 통과
**PR**: `feat/dashboard-main` → main

### 3단계: 매매 내역
- [ ] 매매 목데이터 200건 생성
- [ ] 매매 로그 테이블 (컬럼, 컬러 코딩, 페이지네이션)
- [ ] 필터 바 (기간, 종목, 유형, 결과)
- [ ] 일별 수익 바 차트 (30일)
- [ ] 빌드 확인 + 린트 통과
**PR**: `feat/trade-history` → main

### 4단계: 봇 관리
- [ ] 봇 카드 리스트 (3개)
- [ ] 봇 상태 뱃지 + 시작/중지 토글
- [ ] 봇 생성 모달/페이지 (전략 선택, 파라미터, 종목, 투자금)
- [ ] 빌드 확인 + 린트 통과
**PR**: `feat/bot-management` → main

### 5단계: 포트폴리오 분석
- [ ] 보유 자산 요약 + 종목별 도넛 차트
- [ ] 누적 수익률 라인 차트
- [ ] 핵심 지표 카드 (MDD, 샤프 비율, Profit Factor 등)
- [ ] 빌드 확인 + 린트 통과
**PR**: `feat/portfolio-analysis` → main
